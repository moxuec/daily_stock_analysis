...
### 修复
- 🐛 **飞书机器人重复执行及 Bot 异步交互**（#693，对应 Issue #685）
  - 飞书 Stream 模式下增加消息去重缓存的并发锁机制，防止高并发下重复提交命令；并补充关闭线程池的回退方案。
  - 为避免长连接阻塞引发飞书不断重试，`/ask` 问股和 `/chat` 对话命令改为严格的后台线程异步执行。
  - 用户提交命令后会立即收到包含 `任务 ID` 的确认回执，AI 思考完成后异步推送到原对话上下文。
